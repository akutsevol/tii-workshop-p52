...
thread_local! {
    static FOO: RefCell<u32> = const { RefCell::new(42) };
}
...
fn test_scope() {
    ...
    rayon::scope(|s| {
        s.spawn(|_s| {
            let iter = (0..10000)
                .enumerate()
                .map(|(a, b)| (format!("index {}", a), b));
            map = Some(HashMap::from_iter(iter));
        });
        s.spawn(|_s| factorial = factorial_sequential(30));
        s.spawn(|_s| other = Some("value"))
    });
