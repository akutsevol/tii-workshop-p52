...
where
    F: Fn(u32) -> u64 + Send + 'static + Sync,
{
    let chunk_size = (values.len() + N - 1) / N; // ensure chunks are evenly distributed
    let mut handles = vec![];
    ...
    for chunk in values.chunks(chunk_size) {
        ...
        let handle = thread::spawn(move || chunk.iter().map(|&v| mapper(v)).sum::<u64>());
        handles.push(handle);
    }
